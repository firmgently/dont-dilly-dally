...
//	FGUtils.makeLocal(DontDillyDally, FGHTMLBuild);
  ...
  dataStoragePossible = function() {
    if(typeof(Storage) === "undefined") {
      logMsg(C.TXT_STORAGE_UNSUPPORTED);
    } else {
      return true;
    }
  };
  ...
  initData = function() {
    ...
    if (!dataRetrieveObject(C.PREFS_STR)) {
			// create default preferences object
      dataStoreObject(C.PREFS_STR, {
        pagetype: C.PAGETYPE_DEFAULT,
        timespan: C.TIMESPAN_DEFAULT,
        dateFormat: C.DATETYPE_DEFAULT,
        totalsToShow: C.SHOWTOTALS_DEFAULT,
        minuteIncrements: C.MINUTEINCREMENTS_DEFAULT
      });
      ...
      dataStoreObject(C.CLIENTSTOTAL_STR, 0);
      dataStoreObject(C.JOBSTOTAL_STR, 0);

			// create new object to store clients and fill it with some defaults
      dataStoreObject(C.CLIENTS_STR, {});
      for (item in C.CLIENT_DEFAULTS) {
        tmp_id = getNextID(C.DATATYPE_CLIENT);
        tmp_ob = C.CLIENT_DEFAULTS[item];
        tmp_ob.id = tmp_id;
        createClientOrJobFromOb(tmp_ob, C.DATATYPE_CLIENT);
      }

			// create new object to store jobs and fill it with some defaults
      dataStoreObject(C.JOBS_STR, {});
      for (item in C.JOB_DEFAULTS) {
        tmp_id = getNextID(C.DATATYPE_JOB);
        tmp_ob = C.JOB_DEFAULTS[item];
        tmp_ob.id = tmp_id;
        createClientOrJobFromOb(tmp_ob, C.DATATYPE_JOB);
      }

			// new empty object to store days
			// (work items are stored in days)
      dataStoreObject(C.DAYS_STR, {});
    } else {
			// customise client/job styles based on existing data
			data_ob = dataRetrieveObject(C.CLIENTS_STR);
			for (item in data_ob) {
				createCSSForClientOrJobFromOb(data_ob[item], C.DATATYPE_CLIENT);
			}
			data_ob = dataRetrieveObject(C.JOBS_STR);
			for (item in data_ob) {
				createCSSForClientOrJobFromOb(data_ob[item], C.DATATYPE_JOB);
			}
		}
  };
  ...
  dataStoreObject = function(category, ob) {
    localStorage.setItem(C.APP_ID + C.DATASTORE_CATEGORY_PREFIX + category, JSON.stringify(ob));
    startCSSAnimation(workingIndicator_el, C.CLASS_ANIM_WORKING);
  };


  dataRetrieveObject = function(category) {
    return JSON.parse(localStorage.getItem(C.APP_ID + C.DATASTORE_CATEGORY_PREFIX + category));
  };
  ...
  dataUpdateObject = function(category, key, value) {
	...
	updateDataFromWorkItemEl = function (el) {
        ...
        isMoneyTaskChk_el = el.getElementsByClassName(C.CLASS_MONEYTIMECHECKBOX)[0],
        clientSelect_el = el.getElementsByClassName(C.CLASS_CLIENTSELECT)[0],
        jobSelect_el = el.getElementsByClassName(C.CLASS_JOBSELECT)[0],
        notesInput_el = el.getElementsByClassName(C.CLASS_NOTESINPUT)[0], 
		...
		days_ar = dataRetrieveObject(C.DAYS_STR);
		day_ob = days_ar[day_str];
		if (day_ob === undefined) { day_ob = {}; }

		workItem_ar = [];
		if (isMoneyTaskChk_el && isMoneyTaskChk_el.checked) {
			workItem_ar[C.DATAINDICES.itemType] = C.ITEMTYPE_MONEY;
		} else {
			workItem_ar[C.DATAINDICES.itemType] = C.ITEMTYPE_TIME;
		}
    workItem_ar[C.DATAINDICES.numberValue] = getNumberDataFromWorkItem(el).value;
  
    if (day_ob[el.id]) {
      previouslySelectedClientID = day_ob[el.id][C.DATAINDICES.clientID];
      previouslySelectedJobID = day_ob[el.id][C.DATAINDICES.jobID];
    }
    if (previouslySelectedClientID !== selectedClientID) {
      updateJobOrClientCount(C.CLIENTS_STR, previouslySelectedClientID, -1);
      updateJobOrClientCount(C.CLIENTS_STR, selectedClientID, 1)
    }
    if (previouslySelectedJobID !== selectedJobID) {
      updateJobOrClientCount(C.JOBS_STR, previouslySelectedJobID, -1);
      updateJobOrClientCount(C.JOBS_STR, selectedJobID, 1);
    }

		workItem_ar[C.DATAINDICES.clientID] = selectedClientID;
		workItem_ar[C.DATAINDICES.jobID] = selectedJobID;
		workItem_ar[C.DATAINDICES.notes] = notesInput_el.value;

		day_ob[el.id] = workItem_ar; // write work item to day
		days_ar[day_str] = day_ob; // write updated day
		dataStoreObject(C.DAYS_STR, days_ar);
	};
  ...
  updateDataFromClientOrJobEl = function(el) {
    ...
    for (i = 0; i < inputNodes.length; i++) {
      ...
      } else if (currentNode.className.indexOf(C.CLASS_COLORPICKER) !== -1) {
    ...
    if (el.id.indexOf(C.CLIENT_STR) !== -1) {
      itemType = C.CLIENTS_STR;
      dataType = C.DATATYPE_CLIENT;
    } else if (el.id.indexOf(C.JOB_STR) !== -1) {
      itemType = C.JOBS_STR;
      dataType = C.DATATYPE_JOB;
    }
  ...
  selectPage = function(pagetype) {
    dataUpdateObject(C.PREFS_STR, "pagetype", pagetype);
    location.hash = pagetype;
    if (pagetype === C.PAGETYPE_TIMESHEETS) {
      removeClassname(loadingIndicator_el, C.CLASS_HIDDEN);
    }
    refreshPage();
  };
  ...
  drawPage = function() {
    ...
    switch (dataRetrieveObject(C.PREFS_STR).pagetype) {
      case C.PAGETYPE_TIMESHEETS:
        document.body.id = C.BODYID_TIMESHEETS;
        fillHTMLFromOb(C.PAGEDATA_TIMESHEETS);
        drawGUIFromAr(C.GUIDATA_TIMESHEETS);
				timesheet_el = document.getElementById(C.EL_ID_TIMESHEETCONTAINER);
        break;
      case C.PAGETYPE_CONFIG:
        document.body.id = C.BODYID_CONFIG;
        fillHTMLFromOb(C.PAGEDATA_CONFIG);
        drawGUIFromAr(C.GUIDATA_CONFIG);
        addClassname(loadingIndicator_el, C.CLASS_HIDDEN);
        break;
      case C.PAGETYPE_JOBSANDCLIENTS:
        document.body.id = C.BODYID_JOBSANDCLIENTS;
        fillHTMLFromOb(C.PAGEDATA_JOBSANDCLIENTS);
        drawGUIFromAr(C.GUIDATA_JOBSANDCLIENTS);
        addClassname(loadingIndicator_el, C.CLASS_HIDDEN);
        break;
      case C.PAGETYPE_PRIVACY:
        document.body.id = C.BODYID_PRIVACY;
        fillHTMLFromOb(C.PAGEDATA_PRIVACY);
        drawGUIFromAr(C.GUIDATA_PRIVACY);
        addClassname(loadingIndicator_el, C.CLASS_HIDDEN);
        break;
      default:
        break;
    ...
    for (i = 0; i < ar.length; i++) {
      ...
      switch (ob.type) {
        ...
        case C.GUITYPE_FORM:
          ...
          break;
        case C.GUITYPE_TEXTINPUT:
          tmp_el = createInputFromOb(ob);
          break;
        case C.GUITYPE_SELECT:
          tmp_el = createSelectFromOb(ob);
          break;
        case C.GUITYPE_RADIOBTN:
          ob.checkIfMatched = dataRetrieveObject(C.PREFS_STR)[ob.id];
          tmp_el = createRadioFromOb(ob);
          break;
        case C.GUITYPE_UL:
          tmp_el = addLIsFromOb(ob);
          break;
        case C.GUITYPE_PARA: // intentional rollthrough
        case C.GUITYPE_SECTION: // intentional rollthrough
        case C.GUITYPE_COL: // intentional rollthrough
        case C.GUITYPE_ROW: // intentional rollthrough
        case C.GUITYPE_HEADING: // intentional rollthrough
        case C.GUITYPE_SPACER:
          tmp_el = createBasicElementFromOb(ob);
          break;
        case C.GUITYPE_COLORPICKER:
          tmp_el = createColorPickerFromOb(ob);
          break;
        case C.GUITYPE_METHODCALL:
          callMethodFromOb(ob);
          break;
        case C.GUITYPE_HELP:
  ...
  drawTotalsContainer = function(data_ob) {
    var row_el, cell_el,
        heading_el:any = document.createElement("h4"),
        table_el:any = document.createElement("table");

    addClassname(table_el, C.CLASS_TOTALSCONTAINER);
    ...
    cell_el.innerHTML = C.HOURSWORKED_STR; 
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = "00:00"; 
    addClassname(cell_el, C.CLASS_TOTALHRSWORKED);
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = C.INCOME_STR; 
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = "00:00";
    addClassname(cell_el, C.CLASS_TOTALINCOME);
    ...
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = C.EXPENDITURE_STR; 
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = "00:00"; 
    addClassname(cell_el, C.CLASS_TOTALSPEND);
    ...
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = C.PROFIT_STR; 
    cell_el = row_el.appendChild(document.createElement("td"));
    cell_el.innerHTML = "00:00"; 
    addClassname(cell_el, C.CLASS_TOTALPROFIT);
  ...
  drawJobsAndClients = function() {
    ...
    item_ar = dataRetrieveObject(C.CLIENTS_STR);
    container_el = document.createElement("ul");
    addClassname(container_el, C.CLIENTS_STR);
    document.getElementById(C.EL_ID_CLIENTSEXISTING).appendChild(container_el);
    for (key in item_ar) {
      drawClientOrJobFromOb(item_ar[key]);
    }

    item_ar = dataRetrieveObject(C.JOBS_STR);
    container_el = document.createElement("ul");
    addClassname(container_el, C.JOBS_STR);
    document.getElementById(C.EL_ID_JOBSEXISTING).appendChild(container_el);
    for (key in item_ar) {
      drawClientOrJobFromOb(item_ar[key]);
    }
  };
  ...
  drawClientOrJobFromOb = function(item) {
    ...
    registerEventHandler(item_el, C.COLORPICKER_CHANGEEVENT_ID, updateColoursFromPickers);
    registerEventHandler(item_el, C.COLORPICKER_CONFIRMEVENT_ID, onColorChangeConfirm);

    if (item.id.indexOf(C.CLIENT_STR) !== -1) {
      addMethodPath = "uk.co.firmgently.DontDillyDally.addClient";
      parent_el = document.getElementById(C.EL_ID_CLIENTSEXISTING).getElementsByTagName("UL")[0];
    } else if (item.id.indexOf(C.JOB_STR) !== -1) {
      addMethodPath = "uk.co.firmgently.DontDillyDally.addJob";
      parent_el = document.getElementById(C.EL_ID_JOBSEXISTING).getElementsByTagName("UL")[0];
    }
    ...
    tmp_el = createButtonFromOb({
      class: C.CLASS_ADDITEMBTN,
      label: C.LABEL_ADDITEM,
      parent: item_el,
    });
    ...
    tmp_el = createInputFromOb({
      class: item.id,
      parent: item_el,
      attributes: { "type": "text", "value": item.name, maxlength: C.MAXLENGTH_INPUT_NAMES },
    });
    attachEventArrayToElement(tmp_el, [
      {
        eventType: "change",
    ...
    // 'remove task' button
    tmp_el = createButtonFromOb({
      class: C.CLASS_REMOVEITEMBTN,
     	label: C.LABEL_REMOVEITEM,
      parent: item_el,
    });
		attachEventArrayToElement(tmp_el, [
			{
    ...
    manualEvent(item_el, C.COLORPICKER_CHANGEEVENT_ID); // ensure input's colours are updated
  };
  ...
  drawNextDay = function() {
    var day_str,
				...
				dayWorkItems, itemID,
				prevDay = new Date(),
        totalsToShow = dataRetrieveObject(C.PREFS_STR).totalsToShow,
        ...
        showMonthTotals = (totalsToShow === "showTotalsMonth" || totalsToShow === "showTotalsWeekAndMonth"), 
				...
				allWorkItems = dataRetrieveObject(C.DAYS_STR),
        percentDrawn = 100 * (tsDaysToDraw / tsDaysToDrawTotal),
        scaledPercent = percentDrawn / C.LOADER_CSSWIDTH_SCALE;
    ...
    //if (curDrawnDay.getDate() === 1 && curDrawnDay.getMonth() > 0) {
      ...
      monthHeader_el.innerHTML = C.MONTH_NAMES[curDrawnDay.getMonth()] + " " + curDrawnDay.getFullYear();
      day_el.appendChild(monthHeader_el);
      if (showMonthTotals && tsDaysToDraw !== tsDaysToDrawTotal) {
        totals_el = document.createElement("li");
        addClassname(totals_el, C.CLASS_TOTALSMONTH);
        tsWorkingFragment.appendChild(totals_el);
        drawTotalsContainer({
          heading: C.MONTH_NAMES[curDrawnDay.getMonth() - 1] + " " + curDrawnDay.getFullYear(),
          parent_el: totals_el,
          endDate: new Date(curDrawnDay.getTime()),
          timeSpan: C.TIMESPAN_MONTH
        });
      }
    }
    ...
    if (significance_str !== "") {
      date_el.innerHTML = "<em>" + curDrawnDay.getWeekDay(1) + "</em>" + getFormattedDate(curDrawnDay, C.DATETYPES[dataRetrieveObject(C.PREFS_STR).dateFormat].label) + "<span>" + significance_str + "</span>";
    } else {
      date_el.innerHTML = "<em>" + curDrawnDay.getWeekDay(1) + "</em>" + getFormattedDate(curDrawnDay, C.DATETYPES[dataRetrieveObject(C.PREFS_STR).dateFormat].label);
    }
    ...
    if (tsDaysToDraw > 1) {
      if (tsDaysToDraw % C.DAYSDRAWN_UPDATE_FREQ === 0) {
      ...
      // add year totals
      totals_el = document.createElement("li");
      addClassname(totals_el, C.CLASS_TOTALSYEAR);
      drawTotalsContainer({
        heading: "timesheet totals",
        //heading: (curDrawnDay.getFullYear() - 1) + " totals",
        parent_el: totals_el,
        endDate: new Date(curDrawnDay.getTime()),
        timeSpan: C.TIMESPAN_YEAR
      });
      tsWorkingFragment.appendChild(totals_el);
  ...
  drawTimesheet = function() {
   ...
   switch(dataRetrieveObject(C.PREFS_STR).timespan) {
      case C.TIMESPAN_WEEK:
        ...
        tsDaysToDraw = C.DAYSINWEEK;
        break;
      case C.TIMESPAN_MONTH:
        curDrawnDay.setDate(1);
        tsDaysToDraw = curDrawnDay.monthDays();
        break;
      case C.TIMESPAN_YEAR:
        curDrawnDay.setMonth(0);
        curDrawnDay.setDate(1);
        tsDaysToDraw = C.DAYSINYEAR;
  ...
  drawUIWorkItem = function(dayDataContainer_el, itemID, itemData_ob) {
    ...
    tmp_el = createSelectFromOb({
      contentType: C.CONTENTTYPE_CLIENTS,
      placeholderText: C.CLIENT_SELECT_PLACEHOLDER,
      options: dataRetrieveObject(C.CLIENTS_STR),
      parent: item_el,
    });
    attachEventArrayToElement(tmp_el, [
        {
		...
		if (itemData_ob && itemData_ob[C.DATAINDICES.clientID]
		&& itemData_ob[C.DATAINDICES.clientID].length > 0) {
			changeSelectByOption(tmp_el, itemData_ob[C.DATAINDICES.clientID]);
      updateJobOrClientCount(C.CLIENTS_STR, itemData_ob[C.DATAINDICES.clientID], 1);
			manualEvent(tmp_el, "change");
		}
		//////////
    // job select/dropdown
    tmp_el = createSelectFromOb({
      contentType: C.CONTENTTYPE_JOBS,
      placeholderText: C.JOB_SELECT_PLACEHOLDER,
      options: dataRetrieveObject(C.JOBS_STR),
      parent: item_el,
    });
    attachEventArrayToElement(tmp_el, [
        {
		...
		if (itemData_ob && itemData_ob[C.DATAINDICES.jobID] && itemData_ob[C.DATAINDICES.jobID].length > 0) {
			changeSelectByOption(tmp_el, itemData_ob[C.DATAINDICES.jobID]);
      updateJobOrClientCount(C.JOBS_STR, itemData_ob[C.DATAINDICES.jobID], 1);
			manualEvent(tmp_el, "change");
		}
		//////////
		// 'money/task' checkbox
    ...
    attachEventArrayToElement(tmp_el, [
		...
		if (itemData_ob && itemData_ob[C.DATAINDICES.itemType] === C.ITEMTYPE_MONEY) {
			...
			tmp_el.checked = true;
      //
      if (itemData_ob && itemData_ob[C.DATAINDICES.numberValue]) {
        tmpNum = parseInt(itemData_ob[C.DATAINDICES.numberValue], 10);
        ...
        numberValue_ar = floatToArray(tmpNum/100); // cash is stored in pennies
			...
			addClassname(item_el, C.CLASS_MONEY);
		} else {
      if (itemData_ob && itemData_ob[C.DATAINDICES.numberValue]) {
        tmpNum = parseInt(itemData_ob[C.DATAINDICES.numberValue], 10);
        ...
        numberValue_ar = [
          ...
          tmpNum % 60
			...
			addClassname(item_el, C.CLASS_HOURS);
    }
    //////////
    ...
    // hours/money big unit spinner input
		if (!itemData_ob || itemData_ob[C.DATAINDICES.itemType] === C.ITEMTYPE_TIME) {
      tmp_ob = C.NUMFIELD_DATA_TIME_BIG;
    } else {
      tmp_ob = C.NUMFIELD_DATA_MONEY_BIG;
    }
    tmp_el = createSpinnerFromOb({
      class: C.CLASS_SPINNER_UNITBIG,
      parent: item_el,
      attributes: tmp_ob,
    });
    attachEventArrayToElement(tmp_el, [
    ...
    // hours/money small unit spinner input
		if (!itemData_ob || itemData_ob[C.DATAINDICES.itemType] === C.ITEMTYPE_TIME) {
      tmp_ob = C.NUMFIELD_DATA_TIME_SMALL;
      switch(dataRetrieveObject(C.PREFS_STR).minuteIncrements) {
        case C.MINUTEINCREMENTS_15:
          tmp_ob.step = 15;
          break;
        case C.MINUTEINCREMENTS_30:
          tmp_ob.step = 30;
          break;
        case C.MINUTEINCREMENTS_1: // intentional rollthrough
        default:
          tmp_ob.step = 1;
          break;
      }
    } else {
      tmp_ob = C.NUMFIELD_DATA_MONEY_SMALL;
    }
    tmp_el = createSpinnerFromOb({
      class: C.CLASS_SPINNER_UNITSMALL,
      parent: item_el,
      attributes: tmp_ob,
    });
    attachEventArrayToElement(tmp_el, [
    ...
    if (getNumberDataFromWorkItem(item_el).value < 0) {
      addClassname(tmp_el.parentNode.parentNode, C.CLASS_NEGATIVE);
    }
    //////////
    // break visibility is controlled with CSS
    item_el.appendChild(document.createElement("br")); 
    ...
    // job/money notes
    tmp_el = createInputFromOb({
      class: C.CLASS_NOTESINPUT,
      parent: item_el,
      attributes: { "type": "text", "placeholder": C.JOBNOTES_PLACEHOLDER, maxlength: C.MAXLENGTH_INPUT_NOTES },
    });
    attachEventArrayToElement(tmp_el, [
      {
        eventType: "change",
		...
		if (itemData_ob && itemData_ob[C.DATAINDICES.notes]) { tmp_el.value = itemData_ob[C.DATAINDICES.notes]; }
		//////////
    // 'remove item' button
    tmp_el = createButtonFromOb({
      class: C.CLASS_REMOVEITEMBTN,
    ...
    if (this.id.indexOf(C.CLIENT_STR) !== -1) {
      type_str = C.CLIENTS_STR;
      msgItemType = C.CLIENT_STR;
    } else if (this.id.indexOf(C.JOB_STR) !== -1) {
      type_str = C.JOBS_STR;
      msgItemType = C.JOB_STR;
    }
      
    data_ob = dataRetrieveObject(type_str);
    usedCount = data_ob[this.id][C.CLIENTSORJOBS_USED];
  ...
  removeWorkItem = function(el) {
    ...
    if (day_ob) {
      ...
      if (workItem_ob) {
        previouslySelectedClientID = workItem_ob[C.DATAINDICES.clientID];
        previouslySelectedJobID = workItem_ob[C.DATAINDICES.jobID];
  ...
  onFileSelect = function(event) {
    ...
    reader.onload = function(event) {
      ...
      if (confirm(C.TXT_LOADFILE_CONFIRM)) {
        currentPage = dataRetrieveObject(C.PREFS_STR).pagetype;
        data_ob = JSON.parse(event.target.result);
        dataStoreObject(C.PREFS_STR, data_ob[C.PREFS_STR]);
        dataStoreObject(C.JOBS_STR, data_ob[C.JOBS_STR]);
        dataStoreObject(C.JOBSTOTAL_STR, data_ob[C.JOBSTOTAL_STR]);
        dataStoreObject(C.CLIENTS_STR, data_ob[C.CLIENTS_STR]);
        dataStoreObject(C.CLIENTSTOTAL_STR, data_ob[C.CLIENTSTOTAL_STR]);
        dataStoreObject(C.DAYS_STR, data_ob[C.DAYS_STR]);
        // stay on same page rather than redirecting to page from stored prefs
        dataUpdateObject(C.PREFS_STR, "pagetype", currentPage);
        initData();
        refreshPage();
      }
		};
  ...
  onClearDataBtnClick = function(event) {
    if (confirm(C.TXT_DELETEDATA_CONFIRM)) {
      localStorage.clear();
      initData();
	...
	onSaveBtnClick = function(event) {
		var obj = {}, data, el;
		obj[C.PREFS_STR] = dataRetrieveObject(C.PREFS_STR);
		obj[C.JOBS_STR] = dataRetrieveObject(C.JOBS_STR);
		obj[C.JOBSTOTAL_STR] = dataRetrieveObject(C.JOBSTOTAL_STR);
		obj[C.CLIENTS_STR] = dataRetrieveObject(C.CLIENTS_STR);
		obj[C.CLIENTSTOTAL_STR] = dataRetrieveObject(C.CLIENTSTOTAL_STR);
		obj[C.DAYS_STR] = dataRetrieveObject(C.DAYS_STR);
		...
		el.href = "data:" + data;
		el.download = C.SAVE_FILENAME + ".txt";
  ...
  onFormClick = function(event) {
    var form = this,
        prefs_ob = dataRetrieveObject(C.PREFS_STR);
    if (form && form.id) {
      switch (form.id) {
        case C.EL_ID_CONFIGFORM:
          ...
          dataStoreObject(C.PREFS_STR, prefs_ob);
          break;
        default:
          break;
      }
  ...
  onUpdateInput = function(event) {
    ...
    switch (dataRetrieveObject(C.PREFS_STR).pagetype) {
      case C.PAGETYPE_TIMESHEETS:
        if (this.className.indexOf(C.CLASS_NOTESINPUT) !== -1) {
          ...
          if (document.body.contains(this)) {
            if (getNumberDataFromWorkItem(workItem).value < 0) {
              addClassname(workItem, C.CLASS_NEGATIVE);
            } else {
              removeClassname(workItem, C.CLASS_NEGATIVE);
            }
          }
          // save data
          ...
          clearTimeout(recalculateTotalsTimer);
          recalculateTotalsTimer = setTimeout(recalculateAllTotals, C.RECALCULATETOTALS_DELAY);
        }
        break;
      case C.PAGETYPE_CONFIG:
        break;
  ...
  onIsMoneyTaskChkChange = function() {
    var checkbox = this.getElementsByClassName(C.CLASS_MONEYTIMECHECKBOX)[0],
        spinners = this.getElementsByClassName(C.CLASS_SPINNER),
        spinnerBig = spinners[0].getElementsByTagName("INPUT")[0],
        spinnerSmall = spinners[1].getElementsByTagName("INPUT")[0],
				notesInput = this.getElementsByClassName(C.CLASS_NOTESINPUT)[0];

    if (checkbox.checked) {
			addClassname(this, C.CLASS_MONEY);
			removeClassname(this, C.CLASS_HOURS);
      notesInput.placeholder = C.MONEYNOTES_PLACEHOLDER;
      spinnerBig.spin_ob = C.NUMFIELD_DATA_MONEY_BIG;
      spinnerSmall.spin_ob = C.NUMFIELD_DATA_MONEY_SMALL;
    } else {
			addClassname(this, C.CLASS_HOURS);
			removeClassname(this, C.CLASS_MONEY);
      notesInput.placeholder = C.JOBNOTES_PLACEHOLDER;
      spinnerBig.spin_ob = C.NUMFIELD_DATA_TIME_BIG;
      spinnerSmall.spin_ob = C.NUMFIELD_DATA_TIME_SMALL;
    }
  ...
  getNumberDataFromWorkItem = function(item_el) {
    var return_ob,
        spinners = item_el.getElementsByClassName(C.CLASS_SPINNER),
    ...
    if (item_el.className.indexOf(C.CLASS_MONEY) !== -1) {
      return_ob = { type: C.ITEMTYPE_MONEY };
    ...
    } else if (item_el.className.indexOf(C.CLASS_HOURS) !== -1) {
      return_ob = {
        type: C.ITEMTYPE_TIME,
  ...
  resetJobAndClientCounts = function() {
    ...
    data_ob = dataRetrieveObject(C.CLIENTS_STR);
    for (item in data_ob) {
      data_ob[item][C.CLIENTSORJOBS_USED] = 0;
    }
    dataStoreObject(C.CLIENTS_STR, data_ob);

    data_ob = dataRetrieveObject(C.JOBS_STR);
    for (item in data_ob) {
      data_ob[item][C.CLIENTSORJOBS_USED] = 0;
    }
    dataStoreObject(C.JOBS_STR, data_ob);
  };
  ...
  updateJobOrClientCount = function(dataType, id, value) {
    ...
    if (id && data_ob[id]) {
      if (!data_ob[id][C.CLIENTSORJOBS_USED]) { data_ob[id][C.CLIENTSORJOBS_USED] = 0; }
      data_ob[id][C.CLIENTSORJOBS_USED] += value;
	...
	getJobOrClientIDFromElement = function(el) {
		...
		if (el) {
			...
			for (i = 0; i < class_ar.length; i++) {
				curClass = class_ar[i];
				if (curClass !== C.CLASS_JOBSELECT && curClass !== C.CLASS_CLIENTSELECT) {
					return curClass;
				}
			}
		}
  ...
  recalculateAllTotals = function() {
    var i, curContainer, curTotalsData,
        totalsContainers = timesheet_el.getElementsByClassName(C.CLASS_TOTALSCONTAINER);
    ...
    for (i = 0; i < totalsContainers.length; i++) {
      ...
      curContainer.getElementsByClassName(C.CLASS_TOTALINCOME)[0].innerHTML = curTotalsData.incomeTotal;
      curContainer.getElementsByClassName(C.CLASS_TOTALSPEND)[0].innerHTML = curTotalsData.spendTotal;
      curContainer.getElementsByClassName(C.CLASS_TOTALHRSWORKED)[0].innerHTML = curTotalsData.timeTotal;
      curContainer.getElementsByClassName(C.CLASS_TOTALPROFIT)[0].innerHTML = curTotalsData.incomeTotal + curTotalsData.spendTotal; // spendTotal is a negative value
    }
  };
  ...
  calculateTotalsFromDateSpan = function(dateEnd, timeSpan) {
    ...
    if (timeSpan === C.TIMESPAN_WEEK) {
			dateStart.setDate(dateStart.getDate() - C.DAYSINWEEK);
		} else if (timeSpan === C.TIMESPAN_MONTH) {
			daysInPrevMonth = new Date(dateStart.getDate() - 1).monthDays();
			dateStart.setDate(dateStart.getDate() - daysInPrevMonth);
		} else if (timeSpan === C.TIMESPAN_YEAR) {
			dateStart.setDate(dateStart.getDate() - C.DAYSINYEAR);
		}
    ...
    for (i = 0; i < daysToCalculate; i++) {
      ...
      if (dayWorkItems) {
          ...
          if (itemCur[C.DATAINDICES.itemType] === C.ITEMTYPE_MONEY) {
            //tmpVal = getNumberDataFromWorkItem(n
            tmpVal = parseInt(itemCur[C.DATAINDICES.numberValue], 10);
            if (tmpVal < 0) {
              return_ob.spendTotal += parseInt(itemCur[C.DATAINDICES.numberValue], 10);
            } else {
              return_ob.incomeTotal += parseInt(itemCur[C.DATAINDICES.numberValue], 10);
            }
          } else if (itemCur[C.DATAINDICES.itemType] === C.ITEMTYPE_TIME) {
            return_ob.timeTotal += parseInt(itemCur[C.DATAINDICES.numberValue], 10);
          }
        }
      }
    ...
    return_ob.timeTotal = padString("" + (tmpVal - (tmpVal % 60)) / 60, C.PADSTR_DOUBLEDIGIT);
    return_ob.timeTotal += C.SEPARATOR_TIME;
    return_ob.timeTotal += padString("" + tmpVal % 60, C.PADSTR_DOUBLEDIGIT);
    return return_ob;
  };
  ...
  addClient = function() {
    var ob = getNewClient();
    createClientOrJobFromOb(ob, C.DATATYPE_CLIENT);
  ...
  addJob = function() {
    var ob = getNewJob();
    createClientOrJobFromOb(ob, C.DATATYPE_JOB);
    drawClientOrJobFromOb(ob);
  };
  ...
  getNextID = function(type) {
		var prefix, name, n;
		if (type === C.DATATYPE_JOB) {
			n = 0 + (dataRetrieveObject(C.JOBSTOTAL_STR)) + 1;
			dataStoreObject(C.JOBSTOTAL_STR, n);
			prefix = C.JOB_STR;
		} else if (type === C.DATATYPE_CLIENT) {
			n = 0 + (dataRetrieveObject(C.CLIENTSTOTAL_STR)) + 1;
			dataStoreObject(C.CLIENTSTOTAL_STR, n);
			prefix = C.CLIENT_STR;
		}
		return prefix + n;
  };
  ...
  createClientOrJobFromOb = function(ob, dataType) {
    ...
    if (dataType === C.DATATYPE_CLIENT) {
      containerObjectName = C.CLIENTS_STR;
    } else if (dataType === C.DATATYPE_JOB) {
      containerObjectName = C.JOBS_STR;
    }
    ...
    if (dataType === C.DATATYPE_CLIENT || dataType === C.DATATYPE_JOB) {
  ...
  getNewClient = function() {
    var id = getNextID(C.DATATYPE_CLIENT);
    return {
      id: id,
      name: id,
  ...
  getNewJob = function() {
    var id = getNextID(C.DATATYPE_JOB);
    return {
      id: id,
      name: id,
  ...
  dayJump = function(event) {
    ...
    if (btnID === C.EL_ID_WEEKNEXTBTN || btnID === C.EL_ID_WEEKPREVBTN) {
        dayNodes = document.getElementsByClassName(C.CLASS_WEEKSTART);
		} else if (btnID === C.EL_ID_MONTHNEXTBTN || btnID === C.EL_ID_MONTHPREVBTN) {
        dayNodes = document.getElementsByClassName(C.CLASS_MONTHSTART);
    }
    
    if (dayNodes.length > 0) {
      if (btnID === C.EL_ID_WEEKNEXTBTN || btnID === C.EL_ID_MONTHNEXTBTN) { // search forward
        ...
        for (i = 0; i < dayNodes.length; i++) {
          day = dayNodes[i];
          if (day.offsetTop > scrollTop + C.DAYJUMP_TOPPOS_BUFFERPX + 1) { break; } 
        }
      } else if (btnID === C.EL_ID_WEEKPREVBTN || btnID === C.EL_ID_MONTHPREVBTN) { // search backward
      ...
      if (day) {
        mainContainer_el.scrollTop = day.offsetTop - C.DAYJUMP_TOPPOS_BUFFERPX;
        //day.scrollIntoView();
        if (btnID === C.EL_ID_WEEKNEXTBTN || btnID === C.EL_ID_WEEKPREVBTN) {
          startCSSAnimation(day, C.CLASS_ANIM_ATTRACT);
          //startCSSAnimation(day.getElementsByTagName("P")[0].getElementsByTagName("SPAN")[0], C.CLASS_ANIM_ATTRACT);
        } else if (btnID === C.EL_ID_MONTHNEXTBTN || btnID === C.EL_ID_MONTHPREVBTN) {
          startCSSAnimation(day.getElementsByTagName("H4")[0], C.CLASS_ANIM_ATTRACT);
        }
    ...
    mainContainer_el.scrollTop = document.getElementsByClassName(C.CLASS_TODAY)[0].offsetTop - C.DAYJUMP_TOPPOS_BUFFERPX;
    startCSSAnimation(document.getElementsByClassName(C.CLASS_TODAY)[0], C.CLASS_ANIM_ATTRACT);
	};
	...
	btnEventAutoRepeat = function(event) {
		clearTimeout(btnEventAutoRepeatTimer);
		btnEventAutoRepeatTimer = setTimeout(function() { manualEvent(event.target, event.type); }, C.AUTOREPEAT_RATE);
	};
  ...
  updateSelected = function() {
    var pagetype = dataRetrieveObject(C.PREFS_STR).pagetype; // refers to current page

    if (pagetype === C.PAGETYPE_TIMESHEETS || pagetype === C.PAGETYPE_JOBSANDCLIENTS) {
      if (this.className.indexOf(C.CLASS_CLIENTSELECT) !== -1) {
        this.className = C.CLASS_CLIENTSELECT + " " + this.value;
      } else if (this.className.indexOf(C.CLASS_JOBSELECT) !== -1) {
        this.className = C.CLASS_JOBSELECT + " " + this.value;
      }
    }
  ...
  doSetup = function() {
    loadingIndicator_el = document.getElementById(C.EL_ID_LOADINGINDICATOR);
		mainContainer_el = document.getElementById(C.EL_ID_MAINCONTAINER);
    ...
    if(dataStoragePossible()) {
      initData();
      drawGUIFromAr(C.GUIDATA_NAVMAIN);
      if (location.hash) {
        selectPage(decodeURIComponent(location.hash.substring(1)));
      } else {
        selectPage(dataRetrieveObject(C.PREFS_STR).pagetype);
      }
    }
