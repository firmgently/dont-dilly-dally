...
---------------------------------------------------------
*/
define("DDDConsts", ["require", "exports"], function (require, exports) {
    "use strict";
    ...
    var CONST = {
        ...
        DAYSDRAWN_UPDATE_FREQ: 12,
        LOADER_CSSWIDTH_SCALE: 2,
...
---------------------------------------------------------
*/
var addCSSRule, getIEVersion, isTouchDevice, registerEventHandler, unregisterEventHandler, stopPropagation, hexOpacityToRGBA, rgbToHex, getRandomHexColor, getRandomContrastingHexColor, hexToRGB_ar, getBrightnessFromRGBAr, floatToArray, createElementWithId, removeClassname, addClassname, getStyle, padString, treatAsUTC, daysBetween, getFormattedDate, getFunctionFromString, getGUID, changeSelectByOption, manualEvent, updateSelectOptionList, getNewStylesheet, makeLocal, isEmpty, logMsg;
...
Date.prototype.getDOY = function () {
    var dayCount = [0, 31, 59, 90, 120, 151, 181, 212, 243, 273, 304, 334], mn = this.getMonth(), dn = this.getDate(), dayOfYear = dayCount[mn] + dn;
...
Date.prototype.getWeekDay = function (length) {
    var ret, weekday = ["Sunday", "Monday", "Tuesday", "Wednesday", "Thursday", "Friday", "Saturday"];
...
Date.prototype.getWeekNumber = function () {
    var yearStart, d = new Date(Date.UTC(this.getFullYear(), this.getMonth(), this.getDate())), dayNum = d.getUTCDay() || 7;
...
getRandomContrastingHexColor = function (hexColor, minContrast) {
    var brightness1, brightness2, c1RGB_ar, hexContrasting, c2RGB_ar = hexToRGB_ar(hexColor), brightness2 = getBrightnessFromRGBAr(c2RGB_ar);
    do {
...
addCSSRule = function (selector, property, newValue, customSheet) {
    var i, curStyleSheet, totalStyleSheets = document.styleSheets.length, newStyle = property + ": " + newValue;
    if (customSheet) {
        try {
...
getFunctionFromString = function (str) {
    var i, scope = window, chain_ar = str.split('.'), chainLength = chain_ar.length - 1;
...
define("FGHTMLBuild", ["require", "exports", "DDDConsts"], function (require, exports, DDDConsts_1) {
    "use strict";
    exports.__esModule = true;
    var SPINNER_REPEAT_RATE = 250, SPINNER_REPEAT_ACCEL = 1.04, SPINNER_CLASSNAME = "spinner", SPINNER_UPBTN_CLASSNAME = "spin-button-up", SPINNER_DOWNBTN_CLASSNAME = "spin-button-down", COLORPICKER_IMG_ID = "color-picker-img", COLORPICKER_CANVAS_ID = "color-picker-canvas", COLORPICKER_IMG_PATH = "/images/wheel.png", HELPITEM_CLASSNAME = "helpItem", fillHTMLFromOb, createButtonFromOb, createRadioFromOb, createCheckboxFromOb, createInputFromOb, createSpinnerFromOb, createSelectFromOb, createFormFromOb, createHelpItemFromOb, addLIsFromOb, createBasicElementFromOb, createColorPickerFromOb, onSpinnerStart, onSpinnerMouseUp, doSpinStep, spinnerTimer, onIncreaseSpinnerMouseDown, onDecreaseSpinnerMouseDown, onColorPickerClick, onColorPickerCanvasClick, onColorPickerClickOutside, onColorPickerCanvasMoveOver, onColorPickerImageLoad, colorPickerImage, colorPickerCanvas, colorPickerClose, colorPickerCanvasClickCount, colorPickerSelectedCurrent;
  ...
  ----------------------------------------------------------------------------- */
    createButtonFromOb = function (ob) {
        ...
        if (ob["class"]) {
            addClassname(button_el, ob["class"]);
        }
        button_el.innerHTML = ob.label;
        button_el.ob = ob;
        if (ob.disabled) {
            button_el.disabled = ob.disabled;
        }
        return button_el;
    };
    createInputFromOb = function (ob) {
        var prop, input_el, label_el, innerHTML = "", parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
        if (ob.id) {
            if (ob.label) {
        ...
        if (ob["class"]) {
            addClassname(input_el, ob["class"]);
        }
    ...
    createSpinnerFromOb = function (ob) {
        var prop, input_el, label_el, up_el, down_el, innerHTML = "", wrapper_el = document.createElement("div"), parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
    ...
    createSelectFromOb = function (ob) {
        var i, prop, select_el, label_el, option_el, clientOrJob_ob, dayPrefix, placeholderOption, parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
        if (ob.id) {
            if (ob.label) {
                label_el = document.createElement("label");
                label_el.innerHTML = ob.label;
        ...
        if (ob["class"]) {
            addClassname(select_el, ob["class"]);
        }
        ...
        if (ob.contentType) { // clients / jobs options get treated differently to normal options
            if (ob.contentType === DDDConsts_1["default"].CONTENTTYPE_CLIENTS) {
                addClassname(select_el, DDDConsts_1["default"].CLASS_CLIENTSELECT);
            }
            else if (ob.contentType === DDDConsts_1["default"].CONTENTTYPE_JOBS) {
                addClassname(select_el, DDDConsts_1["default"].CLASS_JOBSELECT);
            }
            for (prop in ob.options) {
                ...
                if (clientOrJob_ob["class"]) {
                    addClassname(option_el, clientOrJob_ob["class"]);
                }
                if (clientOrJob_ob.id) {
                    addClassname(option_el, clientOrJob_ob.id);
                }
            }
        }
        else { // normal options
    ...
    createCheckboxFromOb = function (ob) {
        var prop, checkbox_el, label_el, div_el, parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
        if (ob.label) {
            label_el = document.createElement("label");
            label_el.innerHTML = ob.label;
            parent_el.appendChild(label_el);
        ...
        if (ob["class"]) {
            addClassname(checkbox_el, ob["class"]);
        }
    ...
    createRadioFromOb = function (ob) {
        var prop, description_el, radio_el, label_el, optionID, optionCount = 0, parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
        if (ob.label) {
        ...
        for (prop in ob.options) {
            ...
            if (ob["class"]) {
                addClassname(radio_el, ob["class"]);
            }
            if (ob.swapLabelPos) {
    ...
    addLIsFromOb = function (ob) {
        var i, li_el, UL_el, parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
        if (ob.id) {
        ...
        addClassname(UL_el, DDDConsts_1["default"].CLASS_ROW);
        if (ob["class"]) {
            addClassname(UL_el, ob["class"]);
        }
        if (ob.ar) {
    ...
    createBasicElementFromOb = function (ob) {
        var el, elType, parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
        switch (ob.type) {
            case DDDConsts_1["default"].GUITYPE_COL:
            case DDDConsts_1["default"].GUITYPE_ROW:
                elType = "span";
                break;
            case DDDConsts_1["default"].GUITYPE_PARA:
                elType = "p";
                break;
            case DDDConsts_1["default"].GUITYPE_SECTION:
                elType = "section";
                break;
            case DDDConsts_1["default"].GUITYPE_HEADING:
                elType = "h" + ob.heirarchy;
                break;
            case DDDConsts_1["default"].GUITYPE_SPACER:
                elType = "hr";
                break;
            default:
                elType = "div";
    ...
    createFormFromOb = function (ob) {
        var i, form_el, parent_el = document.getElementById(ob.parent);
        if (ob.id) {
        ...
        if (ob["class"]) {
            addClassname(form_el, ob["class"]);
        }
        if (ob.title) {
            form_el.innerHTML = "<h2>" + ob.title + "</h2>";
        }
        if (ob.hidden) {
    ...
    createColorPickerFromOb = function (ob) {
        var el, label_el, parent_el = (typeof ob.parent == "string") ? document.getElementById(ob.parent) : ob.parent;
    ...
    onColorPickerCanvasClick = function (event) {
        var activeAreaClicked = false, pixelData = colorPickerCanvas.getContext('2d').getImageData(event.offsetX, event.offsetY, 1, 1).data;
        if (pixelData[3] > 0) {
            activeAreaClicked = true;
        }
        if (activeAreaClicked) {
            manualEvent(colorPickerSelectedCurrent.parentNode, DDDConsts_1["default"].COLORPICKER_CONFIRMEVENT_ID);
            colorPickerClose();
        }
        else {
    ...
    onColorPickerCanvasMoveOver = function (event) {
        ...
        manualEvent(colorPickerSelectedCurrent.parentNode, DDDConsts_1["default"].COLORPICKER_CHANGEEVENT_ID);
        if (pixelData[3] > 0) {
...
---------------------------------------------------------
*/
define("DontDillyDally", ["require", "exports", "DDDConsts"], function (require, exports, DDDConsts_2) {
    "use strict";
    exports.__esModule = true;
    /* ---------------------------------------------------------------------------
          declare / init vars
      ...
      --------------------------------------------------------------------------- */
    var key, dateDisplayStart, dateDisplaySelected, dateToday, tsDaysToDraw, tsDaysToDrawTotal, curDrawnDay, tsWorkingFragment, // ts - timesheet
    ...
    // callbacks
    onFormSubmit, onUpdateInput, onIsMoneyTaskChkChange, onClearDataBtnClick, onSaveBtnClick, onColorChangeConfirm, onFormClick, onScroll, onFileSelect, 
    // other methods
    doSetup, selectPage, refreshPage, drawPage, clearPage, drawGUIFromAr, btnEventAutoRepeat, btnEventAutoRepeatStop, btnEventAutoRepeatStart, recalculateAllTotals, calculateTotalsFromDateSpan, getNumberDataFromWorkItem, addItem, removeItem, addClient, removeClientOrJob, addJob, attachEventArrayToElement, callMethodsFromObOnElement, callMethodFromOb, drawTimesheet, drawNextDay, drawJobsAndClients, drawClientOrJobFromOb, drawTotalsContainer, getNextID, getNewClient, getNewJob, resetJobAndClientCounts, updateJobOrClientCount, navClick, todayClick, dayJump, startCSSAnimation, dataStoragePossible, initData, dataStoreObject, dataRetrieveObject, dataUpdateObject, clientAndJobStyleSheet, createClientOrJobFromOb, createCSSForClientOrJobFromOb, getJobOrClientIDFromElement, updateDataFromWorkItemEl, updateDataFromClientOrJobEl, updateColoursFromPickers, updateSelected, drawUIWorkItem, removeWorkItem;
    /* ---------------------------------------------------------------------------
      ...
      --------------------------------------------------------------------------- */
    //	FGUtils.makeLocal(DontDillyDally, DDDConsts);
    //	FGUtils.makeLocal(DontDillyDally, FGUtils);
    //	FGUtils.makeLocal(DontDillyDally, FGHTMLBuild);
    /* -----------------------------------------------------------------------------
      data access/storage related
    ----------------------------------------------------------------------------- */
    dataStoragePossible = function () {
        if (typeof (Storage) === "undefined") {
            logMsg(DDDConsts_2["default"].TXT_STORAGE_UNSUPPORTED);
        }
        else {
            return true;
        }
    ...
    initData = function () {
        ...
        if (!dataRetrieveObject(DDDConsts_2["default"].PREFS_STR)) {
            // create default preferences object
            dataStoreObject(DDDConsts_2["default"].PREFS_STR, {
                pagetype: DDDConsts_2["default"].PAGETYPE_DEFAULT,
                timespan: DDDConsts_2["default"].TIMESPAN_DEFAULT,
                dateFormat: DDDConsts_2["default"].DATETYPE_DEFAULT,
                totalsToShow: DDDConsts_2["default"].SHOWTOTALS_DEFAULT,
                minuteIncrements: DDDConsts_2["default"].MINUTEINCREMENTS_DEFAULT
            });
            ...
            dataStoreObject(DDDConsts_2["default"].CLIENTSTOTAL_STR, 0);
            dataStoreObject(DDDConsts_2["default"].JOBSTOTAL_STR, 0);
            // create new object to store clients and fill it with some defaults
            dataStoreObject(DDDConsts_2["default"].CLIENTS_STR, {});
            for (item in DDDConsts_2["default"].CLIENT_DEFAULTS) {
                tmp_id = getNextID(DDDConsts_2["default"].DATATYPE_CLIENT);
                tmp_ob = DDDConsts_2["default"].CLIENT_DEFAULTS[item];
                tmp_ob.id = tmp_id;
                createClientOrJobFromOb(tmp_ob, DDDConsts_2["default"].DATATYPE_CLIENT);
            }
            // create new object to store jobs and fill it with some defaults
            dataStoreObject(DDDConsts_2["default"].JOBS_STR, {});
            for (item in DDDConsts_2["default"].JOB_DEFAULTS) {
                tmp_id = getNextID(DDDConsts_2["default"].DATATYPE_JOB);
                tmp_ob = DDDConsts_2["default"].JOB_DEFAULTS[item];
                tmp_ob.id = tmp_id;
                createClientOrJobFromOb(tmp_ob, DDDConsts_2["default"].DATATYPE_JOB);
            }
            // new empty object to store days
            // (work items are stored in days)
            dataStoreObject(DDDConsts_2["default"].DAYS_STR, {});
        }
        else {
            // customise client/job styles based on existing data
            data_ob = dataRetrieveObject(DDDConsts_2["default"].CLIENTS_STR);
            for (item in data_ob) {
                createCSSForClientOrJobFromOb(data_ob[item], DDDConsts_2["default"].DATATYPE_CLIENT);
            }
            data_ob = dataRetrieveObject(DDDConsts_2["default"].JOBS_STR);
            for (item in data_ob) {
                createCSSForClientOrJobFromOb(data_ob[item], DDDConsts_2["default"].DATATYPE_JOB);
            }
        }
    };
    dataStoreObject = function (category, ob) {
        localStorage.setItem(DDDConsts_2["default"].APP_ID + DDDConsts_2["default"].DATASTORE_CATEGORY_PREFIX + category, JSON.stringify(ob));
        startCSSAnimation(workingIndicator_el, DDDConsts_2["default"].CLASS_ANIM_WORKING);
    };
    dataRetrieveObject = function (category) {
        return JSON.parse(localStorage.getItem(DDDConsts_2["default"].APP_ID + DDDConsts_2["default"].DATASTORE_CATEGORY_PREFIX + category));
    };
    dataUpdateObject = function (category, key, value) {
        var keyCurrent, ob = dataRetrieveObject(category);
    ...
    updateDataFromWorkItemEl = function (el) {
        var days_ar, day_ob, workItem_ar, previouslySelectedClientID, previouslySelectedJobID, isMoneyTaskChk_el = el.getElementsByClassName(DDDConsts_2["default"].CLASS_MONEYTIMECHECKBOX)[0], clientSelect_el = el.getElementsByClassName(DDDConsts_2["default"].CLASS_CLIENTSELECT)[0], jobSelect_el = el.getElementsByClassName(DDDConsts_2["default"].CLASS_JOBSELECT)[0], notesInput_el = el.getElementsByClassName(DDDConsts_2["default"].CLASS_NOTESINPUT)[0], selectedClientID = getJobOrClientIDFromElement(clientSelect_el), selectedJobID = getJobOrClientIDFromElement(jobSelect_el), day_el = el.parentNode.parentNode, day_str = day_el.id;
        days_ar = dataRetrieveObject(DDDConsts_2["default"].DAYS_STR);
        ...
        if (isMoneyTaskChk_el && isMoneyTaskChk_el.checked) {
            workItem_ar[DDDConsts_2["default"].DATAINDICES.itemType] = DDDConsts_2["default"].ITEMTYPE_MONEY;
        }
        else {
            workItem_ar[DDDConsts_2["default"].DATAINDICES.itemType] = DDDConsts_2["default"].ITEMTYPE_TIME;
        }
        workItem_ar[DDDConsts_2["default"].DATAINDICES.numberValue] = getNumberDataFromWorkItem(el).value;
        if (day_ob[el.id]) {
            previouslySelectedClientID = day_ob[el.id][DDDConsts_2["default"].DATAINDICES.clientID];
            previouslySelectedJobID = day_ob[el.id][DDDConsts_2["default"].DATAINDICES.jobID];
        }
        if (previouslySelectedClientID !== selectedClientID) {
            updateJobOrClientCount(DDDConsts_2["default"].CLIENTS_STR, previouslySelectedClientID, -1);
            updateJobOrClientCount(DDDConsts_2["default"].CLIENTS_STR, selectedClientID, 1);
        }
        if (previouslySelectedJobID !== selectedJobID) {
            updateJobOrClientCount(DDDConsts_2["default"].JOBS_STR, previouslySelectedJobID, -1);
            updateJobOrClientCount(DDDConsts_2["default"].JOBS_STR, selectedJobID, 1);
        }
        workItem_ar[DDDConsts_2["default"].DATAINDICES.clientID] = selectedClientID;
        workItem_ar[DDDConsts_2["default"].DATAINDICES.jobID] = selectedJobID;
        workItem_ar[DDDConsts_2["default"].DATAINDICES.notes] = notesInput_el.value;
        day_ob[el.id] = workItem_ar; // write work item to day
        days_ar[day_str] = day_ob; // write updated day
        dataStoreObject(DDDConsts_2["default"].DAYS_STR, days_ar);
    };
    updateDataFromClientOrJobEl = function (el) {
        var i, inputNodes, currentNode, fg_el, bg_el, textInput_el, items_ob, itemType, dataType, update_ob;
        ...
        for (i = 0; i < inputNodes.length; i++) {
            ...
            else if (currentNode.className.indexOf(DDDConsts_2["default"].CLASS_COLORPICKER) !== -1) {
        ...
        if (el.id.indexOf(DDDConsts_2["default"].CLIENT_STR) !== -1) {
            itemType = DDDConsts_2["default"].CLIENTS_STR;
            dataType = DDDConsts_2["default"].DATATYPE_CLIENT;
        }
        else if (el.id.indexOf(DDDConsts_2["default"].JOB_STR) !== -1) {
            itemType = DDDConsts_2["default"].JOBS_STR;
            dataType = DDDConsts_2["default"].DATATYPE_JOB;
        }
    ...
    selectPage = function (pagetype) {
        dataUpdateObject(DDDConsts_2["default"].PREFS_STR, "pagetype", pagetype);
        location.hash = pagetype;
        if (pagetype === DDDConsts_2["default"].PAGETYPE_TIMESHEETS) {
            removeClassname(loadingIndicator_el, DDDConsts_2["default"].CLASS_HIDDEN);
        }
        refreshPage();
    };
    ...
    drawPage = function () {
        ...
        switch (dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).pagetype) {
            case DDDConsts_2["default"].PAGETYPE_TIMESHEETS:
                document.body.id = DDDConsts_2["default"].BODYID_TIMESHEETS;
                fillHTMLFromOb(DDDConsts_2["default"].PAGEDATA_TIMESHEETS);
                drawGUIFromAr(DDDConsts_2["default"].GUIDATA_TIMESHEETS);
                timesheet_el = document.getElementById(DDDConsts_2["default"].EL_ID_TIMESHEETCONTAINER);
                break;
            case DDDConsts_2["default"].PAGETYPE_CONFIG:
                document.body.id = DDDConsts_2["default"].BODYID_CONFIG;
                fillHTMLFromOb(DDDConsts_2["default"].PAGEDATA_CONFIG);
                drawGUIFromAr(DDDConsts_2["default"].GUIDATA_CONFIG);
                addClassname(loadingIndicator_el, DDDConsts_2["default"].CLASS_HIDDEN);
                break;
            case DDDConsts_2["default"].PAGETYPE_JOBSANDCLIENTS:
                document.body.id = DDDConsts_2["default"].BODYID_JOBSANDCLIENTS;
                fillHTMLFromOb(DDDConsts_2["default"].PAGEDATA_JOBSANDCLIENTS);
                drawGUIFromAr(DDDConsts_2["default"].GUIDATA_JOBSANDCLIENTS);
                addClassname(loadingIndicator_el, DDDConsts_2["default"].CLASS_HIDDEN);
                break;
            case DDDConsts_2["default"].PAGETYPE_PRIVACY:
                document.body.id = DDDConsts_2["default"].BODYID_PRIVACY;
                fillHTMLFromOb(DDDConsts_2["default"].PAGEDATA_PRIVACY);
                drawGUIFromAr(DDDConsts_2["default"].GUIDATA_PRIVACY);
                addClassname(loadingIndicator_el, DDDConsts_2["default"].CLASS_HIDDEN);
                break;
            default:
                break;
    ...
    drawGUIFromAr = function (ar) {
        ...
        for (i = 0; i < ar.length; i++) {
            ob = ar[i];
            switch (ob.type) {
                case DDDConsts_2["default"].GUITYPE_BTN:
                    tmp_el = createButtonFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_FORM:
                    tmp_el = createFormFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_TEXTINPUT:
                    tmp_el = createInputFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_SELECT:
                    tmp_el = createSelectFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_RADIOBTN:
                    ob.checkIfMatched = dataRetrieveObject(DDDConsts_2["default"].PREFS_STR)[ob.id];
                    tmp_el = createRadioFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_UL:
                    tmp_el = addLIsFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_PARA: // intentional rollthrough
                case DDDConsts_2["default"].GUITYPE_SECTION: // intentional rollthrough
                case DDDConsts_2["default"].GUITYPE_COL: // intentional rollthrough
                case DDDConsts_2["default"].GUITYPE_ROW: // intentional rollthrough
                case DDDConsts_2["default"].GUITYPE_HEADING: // intentional rollthrough
                case DDDConsts_2["default"].GUITYPE_SPACER:
                    tmp_el = createBasicElementFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_COLORPICKER:
                    tmp_el = createColorPickerFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_METHODCALL:
                    callMethodFromOb(ob);
                    break;
                case DDDConsts_2["default"].GUITYPE_HELP:
    ...
    drawTotalsContainer = function (data_ob) {
        var row_el, cell_el, heading_el = document.createElement("h4"), table_el = document.createElement("table");
        addClassname(table_el, DDDConsts_2["default"].CLASS_TOTALSCONTAINER);
        ...
        cell_el.innerHTML = DDDConsts_2["default"].HOURSWORKED_STR;
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = "00:00";
        addClassname(cell_el, DDDConsts_2["default"].CLASS_TOTALHRSWORKED);
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = DDDConsts_2["default"].INCOME_STR;
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = "00:00";
        addClassname(cell_el, DDDConsts_2["default"].CLASS_TOTALINCOME);
        row_el = table_el.appendChild(document.createElement("tr"));
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = DDDConsts_2["default"].EXPENDITURE_STR;
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = "00:00";
        addClassname(cell_el, DDDConsts_2["default"].CLASS_TOTALSPEND);
        row_el = table_el.appendChild(document.createElement("tr"));
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = DDDConsts_2["default"].PROFIT_STR;
        cell_el = row_el.appendChild(document.createElement("td"));
        cell_el.innerHTML = "00:00";
        addClassname(cell_el, DDDConsts_2["default"].CLASS_TOTALPROFIT);
        data_ob.parent_el.appendChild(table_el);
    };
    drawJobsAndClients = function () {
        var container_el, key, item_ar;
        item_ar = dataRetrieveObject(DDDConsts_2["default"].CLIENTS_STR);
        container_el = document.createElement("ul");
        addClassname(container_el, DDDConsts_2["default"].CLIENTS_STR);
        document.getElementById(DDDConsts_2["default"].EL_ID_CLIENTSEXISTING).appendChild(container_el);
        for (key in item_ar) {
            drawClientOrJobFromOb(item_ar[key]);
        }
        item_ar = dataRetrieveObject(DDDConsts_2["default"].JOBS_STR);
        container_el = document.createElement("ul");
        addClassname(container_el, DDDConsts_2["default"].JOBS_STR);
        document.getElementById(DDDConsts_2["default"].EL_ID_JOBSEXISTING).appendChild(container_el);
        for (key in item_ar) {
            drawClientOrJobFromOb(item_ar[key]);
        }
    };
    drawClientOrJobFromOb = function (item) {
        ...
        registerEventHandler(item_el, DDDConsts_2["default"].COLORPICKER_CHANGEEVENT_ID, updateColoursFromPickers);
        registerEventHandler(item_el, DDDConsts_2["default"].COLORPICKER_CONFIRMEVENT_ID, onColorChangeConfirm);
        if (item.id.indexOf(DDDConsts_2["default"].CLIENT_STR) !== -1) {
            addMethodPath = "uk.co.firmgently.DontDillyDally.addClient";
            parent_el = document.getElementById(DDDConsts_2["default"].EL_ID_CLIENTSEXISTING).getElementsByTagName("UL")[0];
        }
        else if (item.id.indexOf(DDDConsts_2["default"].JOB_STR) !== -1) {
            addMethodPath = "uk.co.firmgently.DontDillyDally.addJob";
            parent_el = document.getElementById(DDDConsts_2["default"].EL_ID_JOBSEXISTING).getElementsByTagName("UL")[0];
        }
        parent_el.appendChild(item_el);
        removeMethodPath = "uk.co.firmgently.DontDillyDally.removeClientOrJob";
        // 'add task' button
        tmp_el = createButtonFromOb({
            "class": DDDConsts_2["default"].CLASS_ADDITEMBTN,
            label: DDDConsts_2["default"].LABEL_ADDITEM,
            parent: item_el
        });
        ...
        tmp_el = createInputFromOb({
            "class": item.id,
            parent: item_el,
            attributes: { "type": "text", "value": item.name, maxlength: DDDConsts_2["default"].MAXLENGTH_INPUT_NAMES }
        });
        attachEventArrayToElement(tmp_el, [
            {
                eventType: "change",
        ...
        // 'remove task' button
        tmp_el = createButtonFromOb({
            "class": DDDConsts_2["default"].CLASS_REMOVEITEMBTN,
            label: DDDConsts_2["default"].LABEL_REMOVEITEM,
            parent: item_el
        });
        attachEventArrayToElement(tmp_el, [
            {
        ...
        manualEvent(item_el, DDDConsts_2["default"].COLORPICKER_CHANGEEVENT_ID); // ensure input's colours are updated
    };
    drawNextDay = function () {
        var day_str, isToday, significance_str, rowClassname, monthHeader_el, day_el, date_el, dayDataContainer_el, totals_el, dayWorkItems, itemID, prevDay = new Date(), totalsToShow = dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).totalsToShow, showWeekTotals = (totalsToShow === "showTotalsWeek" || totalsToShow === "showTotalsWeekAndMonth"), showMonthTotals = (totalsToShow === "showTotalsMonth" || totalsToShow === "showTotalsWeekAndMonth"), weekStartDay = 1, // 0 = Sunday, 1 = Monday etc
        allWorkItems = dataRetrieveObject(DDDConsts_2["default"].DAYS_STR), percentDrawn = 100 * (tsDaysToDraw / tsDaysToDrawTotal), scaledPercent = percentDrawn / DDDConsts_2["default"].LOADER_CSSWIDTH_SCALE;
        ...
        if (isToday) {
            rowClassname += DDDConsts_2["default"].CLASS_TODAY + " ";
        }
        ...
        if (curDrawnDay.getDate() === 1) {
            ...
            monthHeader_el.innerHTML = DDDConsts_2["default"].MONTH_NAMES[curDrawnDay.getMonth()] + " " + curDrawnDay.getFullYear();
            day_el.appendChild(monthHeader_el);
            if (showMonthTotals && tsDaysToDraw !== tsDaysToDrawTotal) {
                totals_el = document.createElement("li");
                addClassname(totals_el, DDDConsts_2["default"].CLASS_TOTALSMONTH);
                tsWorkingFragment.appendChild(totals_el);
                drawTotalsContainer({
                    heading: DDDConsts_2["default"].MONTH_NAMES[curDrawnDay.getMonth() - 1] + " " + curDrawnDay.getFullYear(),
                    parent_el: totals_el,
                    endDate: new Date(curDrawnDay.getTime()),
                    timeSpan: DDDConsts_2["default"].TIMESPAN_MONTH
                });
            }
        }
        ...
        if (significance_str !== "") {
            date_el.innerHTML = "<em>" + curDrawnDay.getWeekDay(1) + "</em>" + getFormattedDate(curDrawnDay, DDDConsts_2["default"].DATETYPES[dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).dateFormat].label) + "<span>" + significance_str + "</span>";
        }
        else {
            date_el.innerHTML = "<em>" + curDrawnDay.getWeekDay(1) + "</em>" + getFormattedDate(curDrawnDay, DDDConsts_2["default"].DATETYPES[dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).dateFormat].label);
        }
        ...
        if (tsDaysToDraw > 1) {
            if (tsDaysToDraw % DDDConsts_2["default"].DAYSDRAWN_UPDATE_FREQ === 0) {
            ...
            // add year totals
            totals_el = document.createElement("li");
            addClassname(totals_el, DDDConsts_2["default"].CLASS_TOTALSYEAR);
            drawTotalsContainer({
                heading: "timesheet totals",
                //heading: (curDrawnDay.getFullYear() - 1) + " totals",
                parent_el: totals_el,
                endDate: new Date(curDrawnDay.getTime()),
                timeSpan: DDDConsts_2["default"].TIMESPAN_YEAR
            });
            tsWorkingFragment.appendChild(totals_el);
            // add fragment to DOM
    ...
    drawTimesheet = function () {
        var i, j, weekdayCur, totals_el, weekStartDay = 1; // 0 = Sunday, 1 = Monday etc
        ...
        switch (dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).timespan) {
            case DDDConsts_2["default"].TIMESPAN_WEEK:
                ...
                tsDaysToDraw = DDDConsts_2["default"].DAYSINWEEK;
                break;
            case DDDConsts_2["default"].TIMESPAN_MONTH:
                curDrawnDay.setDate(1);
                tsDaysToDraw = curDrawnDay.monthDays();
                break;
            case DDDConsts_2["default"].TIMESPAN_YEAR:
                curDrawnDay.setMonth(0);
                curDrawnDay.setDate(1);
                tsDaysToDraw = DDDConsts_2["default"].DAYSINYEAR;
                if (curDrawnDay.isLeapYear()) {
                    tsDaysToDraw += 1;
                }
                break;
            default:
    ...
    drawUIWorkItem = function (dayDataContainer_el, itemID, itemData_ob) {
        var money_el, tmp_ob, tmp_el, item_el, wrappedCheckbox_el, tmpNum, numberValue_ar, day_el = dayDataContainer_el.parentNode;
        if (itemID === undefined) {
            itemID = getGUID();
        }
        ...
        tmp_el = createSelectFromOb({
            contentType: DDDConsts_2["default"].CONTENTTYPE_CLIENTS,
            placeholderText: DDDConsts_2["default"].CLIENT_SELECT_PLACEHOLDER,
            options: dataRetrieveObject(DDDConsts_2["default"].CLIENTS_STR),
            parent: item_el
        });
        attachEventArrayToElement(tmp_el, [
            {
        ...
        if (itemData_ob && itemData_ob[DDDConsts_2["default"].DATAINDICES.clientID]
            && itemData_ob[DDDConsts_2["default"].DATAINDICES.clientID].length > 0) {
            changeSelectByOption(tmp_el, itemData_ob[DDDConsts_2["default"].DATAINDICES.clientID]);
            updateJobOrClientCount(DDDConsts_2["default"].CLIENTS_STR, itemData_ob[DDDConsts_2["default"].DATAINDICES.clientID], 1);
            manualEvent(tmp_el, "change");
        }
        //////////
        // job select/dropdown
        tmp_el = createSelectFromOb({
            contentType: DDDConsts_2["default"].CONTENTTYPE_JOBS,
            placeholderText: DDDConsts_2["default"].JOB_SELECT_PLACEHOLDER,
            options: dataRetrieveObject(DDDConsts_2["default"].JOBS_STR),
            parent: item_el
        });
        attachEventArrayToElement(tmp_el, [
            {
        ...
        if (itemData_ob && itemData_ob[DDDConsts_2["default"].DATAINDICES.jobID] && itemData_ob[DDDConsts_2["default"].DATAINDICES.jobID].length > 0) {
            changeSelectByOption(tmp_el, itemData_ob[DDDConsts_2["default"].DATAINDICES.jobID]);
            updateJobOrClientCount(DDDConsts_2["default"].JOBS_STR, itemData_ob[DDDConsts_2["default"].DATAINDICES.jobID], 1);
            manualEvent(tmp_el, "change");
        }
        //////////
        // 'money/task' checkbox
        ...
        if (itemData_ob && itemData_ob[DDDConsts_2["default"].DATAINDICES.itemType] === DDDConsts_2["default"].ITEMTYPE_MONEY) {
            ...
            tmp_el.checked = true;
            //
            if (itemData_ob && itemData_ob[DDDConsts_2["default"].DATAINDICES.numberValue]) {
                tmpNum = parseInt(itemData_ob[DDDConsts_2["default"].DATAINDICES.numberValue], 10);
                //numberValue_ar = itemData_ob[C.DATAINDICES.numberValue].split(C.SEPARATOR_CASH);
                // TODO optimise this (following)
                numberValue_ar = floatToArray(tmpNum / 100); // cash is stored in pennies
            }
            addClassname(item_el, DDDConsts_2["default"].CLASS_MONEY);
        }
        else {
            if (itemData_ob && itemData_ob[DDDConsts_2["default"].DATAINDICES.numberValue]) {
                tmpNum = parseInt(itemData_ob[DDDConsts_2["default"].DATAINDICES.numberValue], 10);
                //numberValue_ar = itemData_ob[C.DATAINDICES.numberValue].split(C.SEPARATOR_TIME);
                numberValue_ar = [
                    (tmpNum - (tmpNum % 60)) / 60,
                    tmpNum % 60
                ];
            }
            addClassname(item_el, DDDConsts_2["default"].CLASS_HOURS);
        }
        //////////
        ...
        if (!itemData_ob || itemData_ob[DDDConsts_2["default"].DATAINDICES.itemType] === DDDConsts_2["default"].ITEMTYPE_TIME) {
            tmp_ob = DDDConsts_2["default"].NUMFIELD_DATA_TIME_BIG;
        }
        else {
            tmp_ob = DDDConsts_2["default"].NUMFIELD_DATA_MONEY_BIG;
        }
        tmp_el = createSpinnerFromOb({
            "class": DDDConsts_2["default"].CLASS_SPINNER_UNITBIG,
            parent: item_el,
            attributes: tmp_ob
        });
        attachEventArrayToElement(tmp_el, [
        ...
        if (!itemData_ob || itemData_ob[DDDConsts_2["default"].DATAINDICES.itemType] === DDDConsts_2["default"].ITEMTYPE_TIME) {
            tmp_ob = DDDConsts_2["default"].NUMFIELD_DATA_TIME_SMALL;
            switch (dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).minuteIncrements) {
                case DDDConsts_2["default"].MINUTEINCREMENTS_15:
                    tmp_ob.step = 15;
                    break;
                case DDDConsts_2["default"].MINUTEINCREMENTS_30:
                    tmp_ob.step = 30;
                    break;
                case DDDConsts_2["default"].MINUTEINCREMENTS_1: // intentional rollthrough
                default:
                    tmp_ob.step = 1;
                    break;
            }
            ...
            tmp_ob = DDDConsts_2["default"].NUMFIELD_DATA_MONEY_SMALL;
        }
        tmp_el = createSpinnerFromOb({
            "class": DDDConsts_2["default"].CLASS_SPINNER_UNITSMALL,
            parent: item_el,
            attributes: tmp_ob
        });
        attachEventArrayToElement(tmp_el, [
        ...
        if (getNumberDataFromWorkItem(item_el).value < 0) {
            addClassname(tmp_el.parentNode.parentNode, DDDConsts_2["default"].CLASS_NEGATIVE);
        }
        //////////
        // break visibility is controlled with CSS
        item_el.appendChild(document.createElement("br"));
        ...
        // job/money notes
        tmp_el = createInputFromOb({
            "class": DDDConsts_2["default"].CLASS_NOTESINPUT,
            parent: item_el,
            attributes: { "type": "text", "placeholder": DDDConsts_2["default"].JOBNOTES_PLACEHOLDER, maxlength: DDDConsts_2["default"].MAXLENGTH_INPUT_NOTES }
        });
        attachEventArrayToElement(tmp_el, [
            {
                eventType: "change",
        ...
        if (itemData_ob && itemData_ob[DDDConsts_2["default"].DATAINDICES.notes]) {
            tmp_el.value = itemData_ob[DDDConsts_2["default"].DATAINDICES.notes];
        }
        //////////
        // 'remove item' button
        tmp_el = createButtonFromOb({
    ...
    removeClientOrJob = function () {
        ...
        if (this.id.indexOf(DDDConsts_2["default"].CLIENT_STR) !== -1) {
            type_str = DDDConsts_2["default"].CLIENTS_STR;
            msgItemType = DDDConsts_2["default"].CLIENT_STR;
        }
        else if (this.id.indexOf(DDDConsts_2["default"].JOB_STR) !== -1) {
            type_str = DDDConsts_2["default"].JOBS_STR;
            msgItemType = DDDConsts_2["default"].JOB_STR;
        }
        data_ob = dataRetrieveObject(type_str);
        usedCount = data_ob[this.id][DDDConsts_2["default"].CLIENTSORJOBS_USED];
    ...
    removeWorkItem = function (el) {
        var workItem_ob, previouslySelectedClientID, previouslySelectedJobID, days_ar = dataRetrieveObject(DDDConsts_2["default"].DAYS_STR), day_ob = days_ar[el.parentNode.parentNode.id];
        if (day_ob) {
            workItem_ob = day_ob[el.id];
            if (workItem_ob) {
                previouslySelectedClientID = workItem_ob[DDDConsts_2["default"].DATAINDICES.clientID];
                previouslySelectedJobID = workItem_ob[DDDConsts_2["default"].DATAINDICES.jobID];
    ...
    onFileSelect = function (event) {
        var data_ob, file = event.target.files[0], // FileList object first item (as only single file)
        reader = new FileReader();
        reader.onload = function (event) {
            var data_ob, currentPage;
            if (confirm(DDDConsts_2["default"].TXT_LOADFILE_CONFIRM)) {
                currentPage = dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).pagetype;
                data_ob = JSON.parse(event.target.result);
                dataStoreObject(DDDConsts_2["default"].PREFS_STR, data_ob[DDDConsts_2["default"].PREFS_STR]);
                dataStoreObject(DDDConsts_2["default"].JOBS_STR, data_ob[DDDConsts_2["default"].JOBS_STR]);
                dataStoreObject(DDDConsts_2["default"].JOBSTOTAL_STR, data_ob[DDDConsts_2["default"].JOBSTOTAL_STR]);
                dataStoreObject(DDDConsts_2["default"].CLIENTS_STR, data_ob[DDDConsts_2["default"].CLIENTS_STR]);
                dataStoreObject(DDDConsts_2["default"].CLIENTSTOTAL_STR, data_ob[DDDConsts_2["default"].CLIENTSTOTAL_STR]);
                dataStoreObject(DDDConsts_2["default"].DAYS_STR, data_ob[DDDConsts_2["default"].DAYS_STR]);
                // stay on same page rather than redirecting to page from stored prefs
                dataUpdateObject(DDDConsts_2["default"].PREFS_STR, "pagetype", currentPage);
                initData();
                refreshPage();
            }
        };
    ...
    onClearDataBtnClick = function (event) {
        if (confirm(DDDConsts_2["default"].TXT_DELETEDATA_CONFIRM)) {
            localStorage.clear();
            initData();
    ...
    onSaveBtnClick = function (event) {
        var obj = {}, data, el;
        obj[DDDConsts_2["default"].PREFS_STR] = dataRetrieveObject(DDDConsts_2["default"].PREFS_STR);
        obj[DDDConsts_2["default"].JOBS_STR] = dataRetrieveObject(DDDConsts_2["default"].JOBS_STR);
        obj[DDDConsts_2["default"].JOBSTOTAL_STR] = dataRetrieveObject(DDDConsts_2["default"].JOBSTOTAL_STR);
        obj[DDDConsts_2["default"].CLIENTS_STR] = dataRetrieveObject(DDDConsts_2["default"].CLIENTS_STR);
        obj[DDDConsts_2["default"].CLIENTSTOTAL_STR] = dataRetrieveObject(DDDConsts_2["default"].CLIENTSTOTAL_STR);
        obj[DDDConsts_2["default"].DAYS_STR] = dataRetrieveObject(DDDConsts_2["default"].DAYS_STR);
        data = "text/json;charset=utf-8," + encodeURIComponent(JSON.stringify(obj));
        // create a phantom element, attach encoded JSON to it
        // add it to DOM, click then remove
        el = document.createElement("a");
        el.href = "data:" + data;
        el.download = DDDConsts_2["default"].SAVE_FILENAME + ".txt";
    ...
    onFormClick = function (event) {
        var form = this, prefs_ob = dataRetrieveObject(DDDConsts_2["default"].PREFS_STR);
        if (form && form.id) {
            switch (form.id) {
                case DDDConsts_2["default"].EL_ID_CONFIGFORM:
                    ...
                    dataStoreObject(DDDConsts_2["default"].PREFS_STR, prefs_ob);
                    break;
                default:
                    break;
            }
    ...
    onUpdateInput = function (event) {
        ...
        switch (dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).pagetype) {
            case DDDConsts_2["default"].PAGETYPE_TIMESHEETS:
                if (this.className.indexOf(DDDConsts_2["default"].CLASS_NOTESINPUT) !== -1) {
                    ...
                    if (document.body.contains(this)) {
                        if (getNumberDataFromWorkItem(workItem).value < 0) {
                            addClassname(workItem, DDDConsts_2["default"].CLASS_NEGATIVE);
                        }
                        else {
                            removeClassname(workItem, DDDConsts_2["default"].CLASS_NEGATIVE);
                        }
                    }
                    // save data
                    if (document.body.contains(this)) {
                    ...
                    clearTimeout(recalculateTotalsTimer);
                    recalculateTotalsTimer = setTimeout(recalculateAllTotals, DDDConsts_2["default"].RECALCULATETOTALS_DELAY);
                }
                break;
            case DDDConsts_2["default"].PAGETYPE_CONFIG:
                break;
    ...
    onIsMoneyTaskChkChange = function () {
        var checkbox = this.getElementsByClassName(DDDConsts_2["default"].CLASS_MONEYTIMECHECKBOX)[0], spinners = this.getElementsByClassName(DDDConsts_2["default"].CLASS_SPINNER), spinnerBig = spinners[0].getElementsByTagName("INPUT")[0], spinnerSmall = spinners[1].getElementsByTagName("INPUT")[0], notesInput = this.getElementsByClassName(DDDConsts_2["default"].CLASS_NOTESINPUT)[0];
        if (checkbox.checked) {
            addClassname(this, DDDConsts_2["default"].CLASS_MONEY);
            removeClassname(this, DDDConsts_2["default"].CLASS_HOURS);
            notesInput.placeholder = DDDConsts_2["default"].MONEYNOTES_PLACEHOLDER;
            spinnerBig.spin_ob = DDDConsts_2["default"].NUMFIELD_DATA_MONEY_BIG;
            spinnerSmall.spin_ob = DDDConsts_2["default"].NUMFIELD_DATA_MONEY_SMALL;
        }
        else {
            addClassname(this, DDDConsts_2["default"].CLASS_HOURS);
            removeClassname(this, DDDConsts_2["default"].CLASS_MONEY);
            notesInput.placeholder = DDDConsts_2["default"].JOBNOTES_PLACEHOLDER;
            spinnerBig.spin_ob = DDDConsts_2["default"].NUMFIELD_DATA_TIME_BIG;
            spinnerSmall.spin_ob = DDDConsts_2["default"].NUMFIELD_DATA_TIME_SMALL;
        }
        if (document.body.contains(this)) {
            updateDataFromWorkItemEl(this);
        }
    };
    /* ---------------------------------------------------------------------------
    ...
    getNumberDataFromWorkItem = function (item_el) {
        var return_ob, spinners = item_el.getElementsByClassName(DDDConsts_2["default"].CLASS_SPINNER), val1 = parseInt(spinners[0].getElementsByTagName("INPUT")[0].value, 10), val2 = parseInt(spinners[1].getElementsByTagName("INPUT")[0].value, 10);
        if (item_el.className.indexOf(DDDConsts_2["default"].CLASS_MONEY) !== -1) {
            return_ob = { type: DDDConsts_2["default"].ITEMTYPE_MONEY };
        ...
        else if (item_el.className.indexOf(DDDConsts_2["default"].CLASS_HOURS) !== -1) {
            return_ob = {
                type: DDDConsts_2["default"].ITEMTYPE_TIME,
    ...
    resetJobAndClientCounts = function () {
        ...
        data_ob = dataRetrieveObject(DDDConsts_2["default"].CLIENTS_STR);
        for (item in data_ob) {
            data_ob[item][DDDConsts_2["default"].CLIENTSORJOBS_USED] = 0;
        }
        dataStoreObject(DDDConsts_2["default"].CLIENTS_STR, data_ob);
        data_ob = dataRetrieveObject(DDDConsts_2["default"].JOBS_STR);
        for (item in data_ob) {
            data_ob[item][DDDConsts_2["default"].CLIENTSORJOBS_USED] = 0;
        }
        dataStoreObject(DDDConsts_2["default"].JOBS_STR, data_ob);
    };
    // dataType: C.CLIENTS_STR or C.JOBS_STR
    // id: client or job id
    ...
    updateJobOrClientCount = function (dataType, id, value) {
        ...
        if (id && data_ob[id]) {
            if (!data_ob[id][DDDConsts_2["default"].CLIENTSORJOBS_USED]) {
                data_ob[id][DDDConsts_2["default"].CLIENTSORJOBS_USED] = 0;
            }
            data_ob[id][DDDConsts_2["default"].CLIENTSORJOBS_USED] += value;
    ...
    getJobOrClientIDFromElement = function (el) {
        ...
        if (el) {
            ...
            for (i = 0; i < class_ar.length; i++) {
                curClass = class_ar[i];
                if (curClass !== DDDConsts_2["default"].CLASS_JOBSELECT && curClass !== DDDConsts_2["default"].CLASS_CLIENTSELECT) {
                    return curClass;
                }
            }
        }
    ...
    recalculateAllTotals = function () {
        var i, curContainer, curTotalsData, totalsContainers = timesheet_el.getElementsByClassName(DDDConsts_2["default"].CLASS_TOTALSCONTAINER);
        for (i = 0; i < totalsContainers.length; i++) {
            ...
            curContainer.getElementsByClassName(DDDConsts_2["default"].CLASS_TOTALINCOME)[0].innerHTML = curTotalsData.incomeTotal;
            curContainer.getElementsByClassName(DDDConsts_2["default"].CLASS_TOTALSPEND)[0].innerHTML = curTotalsData.spendTotal;
            curContainer.getElementsByClassName(DDDConsts_2["default"].CLASS_TOTALHRSWORKED)[0].innerHTML = curTotalsData.timeTotal;
            curContainer.getElementsByClassName(DDDConsts_2["default"].CLASS_TOTALPROFIT)[0].innerHTML = curTotalsData.incomeTotal + curTotalsData.spendTotal; // spendTotal is a negative value
        }
    };
    calculateTotalsFromDateSpan = function (dateEnd, timeSpan) {
        var i, j, daysToCalculate, dayCur, day_str, tmpVal, dayWorkItems, itemID, itemCur, daysInPrevMonth, dateStart, return_ob = { timeTotal: 0, incomeTotal: 0, spendTotal: 0 }, allWorkItems = dataRetrieveObject(DDDConsts_2["default"].DAYS_STR);
        dateStart = new Date(dateEnd.getTime());
        if (timeSpan === DDDConsts_2["default"].TIMESPAN_WEEK) {
            dateStart.setDate(dateStart.getDate() - DDDConsts_2["default"].DAYSINWEEK);
        }
        else if (timeSpan === DDDConsts_2["default"].TIMESPAN_MONTH) {
            daysInPrevMonth = new Date(dateStart.getDate() - 1).monthDays();
            dateStart.setDate(dateStart.getDate() - daysInPrevMonth);
        }
        else if (timeSpan === DDDConsts_2["default"].TIMESPAN_YEAR) {
            dateStart.setDate(dateStart.getDate() - DDDConsts_2["default"].DAYSINYEAR);
        }
        ...
        for (i = 0; i < daysToCalculate; i++) {
            ...
            if (dayWorkItems) {
                for (itemID in dayWorkItems) {
                    ...
                    if (itemCur[DDDConsts_2["default"].DATAINDICES.itemType] === DDDConsts_2["default"].ITEMTYPE_MONEY) {
                        //tmpVal = getNumberDataFromWorkItem(n
                        tmpVal = parseInt(itemCur[DDDConsts_2["default"].DATAINDICES.numberValue], 10);
                        if (tmpVal < 0) {
                            return_ob.spendTotal += parseInt(itemCur[DDDConsts_2["default"].DATAINDICES.numberValue], 10);
                        }
                        else {
                            return_ob.incomeTotal += parseInt(itemCur[DDDConsts_2["default"].DATAINDICES.numberValue], 10);
                        }
                    }
                    else if (itemCur[DDDConsts_2["default"].DATAINDICES.itemType] === DDDConsts_2["default"].ITEMTYPE_TIME) {
                        return_ob.timeTotal += parseInt(itemCur[DDDConsts_2["default"].DATAINDICES.numberValue], 10);
                    }
                }
            }
        ...
        return_ob.timeTotal = padString("" + (tmpVal - (tmpVal % 60)) / 60, DDDConsts_2["default"].PADSTR_DOUBLEDIGIT);
        return_ob.timeTotal += DDDConsts_2["default"].SEPARATOR_TIME;
        return_ob.timeTotal += padString("" + tmpVal % 60, DDDConsts_2["default"].PADSTR_DOUBLEDIGIT);
        return return_ob;
    };
    ...
    addClient = function () {
        var ob = getNewClient();
        createClientOrJobFromOb(ob, DDDConsts_2["default"].DATATYPE_CLIENT);
        drawClientOrJobFromOb(ob);
    };
    addJob = function () {
        var ob = getNewJob();
        createClientOrJobFromOb(ob, DDDConsts_2["default"].DATATYPE_JOB);
        drawClientOrJobFromOb(ob);
    };
    // get next available ID for job or client
    getNextID = function (type) {
        var prefix, name, n;
        if (type === DDDConsts_2["default"].DATATYPE_JOB) {
            n = 0 + (dataRetrieveObject(DDDConsts_2["default"].JOBSTOTAL_STR)) + 1;
            dataStoreObject(DDDConsts_2["default"].JOBSTOTAL_STR, n);
            prefix = DDDConsts_2["default"].JOB_STR;
        }
        else if (type === DDDConsts_2["default"].DATATYPE_CLIENT) {
            n = 0 + (dataRetrieveObject(DDDConsts_2["default"].CLIENTSTOTAL_STR)) + 1;
            dataStoreObject(DDDConsts_2["default"].CLIENTSTOTAL_STR, n);
            prefix = DDDConsts_2["default"].CLIENT_STR;
        }
        return prefix + n;
    };
    createClientOrJobFromOb = function (ob, dataType) {
        ...
        if (dataType === DDDConsts_2["default"].DATATYPE_CLIENT) {
            containerObjectName = DDDConsts_2["default"].CLIENTS_STR;
        }
        else if (dataType === DDDConsts_2["default"].DATATYPE_JOB) {
            containerObjectName = DDDConsts_2["default"].JOBS_STR;
        }
    ...
    createCSSForClientOrJobFromOb = function (ob, dataType) {
        var colorPickerFGSelector, colorPickerBGSelector, selector = "." + ob.id + ", " +
            "." + ob.id + ":hover, " +
            "." + ob.id + ":active";
        // add main CSS for eg. timesheets page
        if (dataType === DDDConsts_2["default"].DATATYPE_CLIENT || dataType === DDDConsts_2["default"].DATATYPE_JOB) {
    ...
    getNewClient = function () {
        var id = getNextID(DDDConsts_2["default"].DATATYPE_CLIENT);
        return {
            id: id,
            name: id,
    ...
    getNewJob = function () {
        var id = getNextID(DDDConsts_2["default"].DATATYPE_JOB);
        return {
            id: id,
            name: id,
    ...
    dayJump = function (event) {
        var i, day, dayNodes, btnID = event.target.id, scrollTop = mainContainer_el.scrollTop;
        if (btnID === DDDConsts_2["default"].EL_ID_WEEKNEXTBTN || btnID === DDDConsts_2["default"].EL_ID_WEEKPREVBTN) {
            dayNodes = document.getElementsByClassName(DDDConsts_2["default"].CLASS_WEEKSTART);
        }
        else if (btnID === DDDConsts_2["default"].EL_ID_MONTHNEXTBTN || btnID === DDDConsts_2["default"].EL_ID_MONTHPREVBTN) {
            dayNodes = document.getElementsByClassName(DDDConsts_2["default"].CLASS_MONTHSTART);
        }
        if (dayNodes.length > 0) {
            if (btnID === DDDConsts_2["default"].EL_ID_WEEKNEXTBTN || btnID === DDDConsts_2["default"].EL_ID_MONTHNEXTBTN) { // search forward
                ...
                for (i = 0; i < dayNodes.length; i++) {
                    day = dayNodes[i];
                    if (day.offsetTop > scrollTop + DDDConsts_2["default"].DAYJUMP_TOPPOS_BUFFERPX + 1) {
                        break;
                    }
                }
            }
            else if (btnID === DDDConsts_2["default"].EL_ID_WEEKPREVBTN || btnID === DDDConsts_2["default"].EL_ID_MONTHPREVBTN) { // search backward
                for (i = dayNodes.length - 1; i >= 0; i--) {
                    day = dayNodes[i];
                    if (day.offsetTop < scrollTop - 1) {
                        break;
                    }
                }
            }
            if (day) {
                mainContainer_el.scrollTop = day.offsetTop - DDDConsts_2["default"].DAYJUMP_TOPPOS_BUFFERPX;
                //day.scrollIntoView();
                if (btnID === DDDConsts_2["default"].EL_ID_WEEKNEXTBTN || btnID === DDDConsts_2["default"].EL_ID_WEEKPREVBTN) {
                    startCSSAnimation(day, DDDConsts_2["default"].CLASS_ANIM_ATTRACT);
                    //startCSSAnimation(day.getElementsByTagName("P")[0].getElementsByTagName("SPAN")[0], C.CLASS_ANIM_ATTRACT);
                }
                else if (btnID === DDDConsts_2["default"].EL_ID_MONTHNEXTBTN || btnID === DDDConsts_2["default"].EL_ID_MONTHPREVBTN) {
                    startCSSAnimation(day.getElementsByTagName("H4")[0], DDDConsts_2["default"].CLASS_ANIM_ATTRACT);
                }
    ...
    todayClick = function (event) {
        //document.getElementsByClassName(C.CLASS_TODAY)[0].scrollIntoView();
        mainContainer_el.scrollTop = document.getElementsByClassName(DDDConsts_2["default"].CLASS_TODAY)[0].offsetTop - DDDConsts_2["default"].DAYJUMP_TOPPOS_BUFFERPX;
        startCSSAnimation(document.getElementsByClassName(DDDConsts_2["default"].CLASS_TODAY)[0], DDDConsts_2["default"].CLASS_ANIM_ATTRACT);
    };
    ...
    btnEventAutoRepeat = function (event) {
        clearTimeout(btnEventAutoRepeatTimer);
        btnEventAutoRepeatTimer = setTimeout(function () { manualEvent(event.target, event.type); }, DDDConsts_2["default"].AUTOREPEAT_RATE);
    };
    ...
    updateSelected = function () {
        var pagetype = dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).pagetype; // refers to current page
        if (pagetype === DDDConsts_2["default"].PAGETYPE_TIMESHEETS || pagetype === DDDConsts_2["default"].PAGETYPE_JOBSANDCLIENTS) {
            if (this.className.indexOf(DDDConsts_2["default"].CLASS_CLIENTSELECT) !== -1) {
                this.className = DDDConsts_2["default"].CLASS_CLIENTSELECT + " " + this.value;
            }
            else if (this.className.indexOf(DDDConsts_2["default"].CLASS_JOBSELECT) !== -1) {
                this.className = DDDConsts_2["default"].CLASS_JOBSELECT + " " + this.value;
            }
        }
        if (pagetype === DDDConsts_2["default"].PAGETYPE_TIMESHEETS) {
            if (document.body.contains(this)) {
    ...
    --------------------------------------------------------------------------- */
    doSetup = function () {
        loadingIndicator_el = document.getElementById(DDDConsts_2["default"].EL_ID_LOADINGINDICATOR);
        mainContainer_el = document.getElementById(DDDConsts_2["default"].EL_ID_MAINCONTAINER);
        ...
        if (dataStoragePossible()) {
            initData();
            drawGUIFromAr(DDDConsts_2["default"].GUIDATA_NAVMAIN);
            if (location.hash) {
                selectPage(decodeURIComponent(location.hash.substring(1)));
            }
            else {
                selectPage(dataRetrieveObject(DDDConsts_2["default"].PREFS_STR).pagetype);
            }
        }
